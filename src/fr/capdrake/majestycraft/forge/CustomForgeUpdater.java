...
public class CustomForgeUpdater 
{
  ...
  public String versionConfig, forgeVersionConfig, mcpVersion;
	...
	{		    			
		        ...
		        options = new UpdaterOptions.UpdaterOptionsBuilder().withReExtractNatives(true).withEnableCurseForgePlugin(true).build();				        
		        
		        switch(versionConfig) 
		        {
	        		case "1.9":
		        		  forgeVersion = new ForgeVersionBuilder(ForgeVersionBuilder.ForgeVersionType.OLD).withForgeVersion(forgeVersionConfig).withVanillaVersion(version).build();
		        		  break;
	        		case "1.10.2":
		        		  forgeVersion = new ForgeVersionBuilder(ForgeVersionBuilder.ForgeVersionType.OLD).withForgeVersion(forgeVersionConfig).withVanillaVersion(version).build();
		        		  break;
		        	case "1.11.2":
		        		  ...
		        		  break;
		        	case "1.12.2":
		        		  forgeVersion = new ForgeVersionBuilder(ForgeVersionBuilder.ForgeVersionType.NEW).withForgeVersion(forgeVersionConfig).withVanillaVersion(version).build();
		        		  break;
    				case "1.13.2":
		        		  forgeVersion = new ForgeVersionBuilder(ForgeVersionBuilder.ForgeVersionType.NEW).withForgeVersion(forgeVersionConfig).withVanillaVersion(version).build();
		        		  break;
